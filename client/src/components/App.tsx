...
function App() {
  ...
  const [currentCoworkingSpace, setCurrentCoworkingSpace] = useState<
    coworkingResultsObject | undefined
  >(undefined);
  ...
  useEffect(() => {
    if (searchedCity) {
      setLoading(true);
      fetchNearbyCoworkingSpaces(
        `coworking_spaces/nearby?location=${searchedCity}`
      )
        .then((data) => {
